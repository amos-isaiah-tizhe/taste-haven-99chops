...
const createNotification = async ({
  ...
  type, title, message, order = null, actionUrl = null,
}) => {
  try {
    const config = NOTIFICATION_CONFIG[type] || NOTIFICATION_CONFIG['custom'];
    return await Notification.create({
      recipient, recipientRole, type, title, message,
      order, actionUrl,
      icon:     config.icon,
...
const notifyAllAdmins = async ({
  ...
  try {
    const admins = await User.find({
      role: { $in: ['admin', 'superadmin'] }, isActive: true,
    }).select('_id');

    const notifications = admins.map(admin => ({
      recipient:     admin._id,
      recipientRole: 'admin',
      type, title, message, order, actionUrl,
      icon:     NOTIFICATION_CONFIG[type]?.icon || 'fa-bell',
...
/* ── Notify customer of order status change + send email ─────── */
const notifyOrderStatus = async (order, status) => {
  ...
  try {
    ...
    if (customer?.email) {
      email.sendOrderStatusUpdate(order, customer, status).catch(e =>
        console.error('[Email] Status email failed:', e.message)
      );
    }
  } catch (e) {
    console.error('[Email] Status email error:', e.message);
  }
};

/* ── Notify admins of new order + send email ─────────────────── */
const notifyNewOrder = async (order) => {
  // In-app notification
  await notifyAllAdmins({
    type:      'new-order-admin',
    title:     `New Order: #${order.orderNumber}`,
    message:   `${order.customerName} placed an order of ₦${order.total.toLocaleString()}. ${order.deliveryType === 'delivery' ? 'Delivery' : 'Pickup'} - ${order.paymentMethod.toUpperCase()}`,
    order:     order._id,
  ...
  try {
    ...
    for (const admin of admins) {
      email.sendNewOrderAdminAlert(order, admin.email).catch(e =>
        console.error('[Email] Admin alert failed:', e.message)
      );
    }
  } catch (e) {
    console.error('[Email] Admin email error:', e.message);
  }
};
...
const notifyPaymentReceived = async (order) => {
  ...
  await createNotification({
    ...
    message:       `Payment of ₦${order.total.toLocaleString()} confirmed for order #${order.orderNumber}.`,
    order:         order._id,
    actionUrl:     `/orders/${order._id}`,
  });
  ...
  try {
    const customer = await User.findById(order.customer).select('email firstName');
    if (customer?.email) {
      email.sendPaymentConfirmed(order, customer).catch(e =>
        console.error('[Email] Payment email failed:', e.message)
      );
    }
  } catch (e) {
    console.error('[Email] Payment email error:', e.message);
  }
};
